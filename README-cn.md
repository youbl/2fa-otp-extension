...
- **码快过期来不及粘贴** —— 悬停在某个验证码上，会同时显示「当前码」「下一码」「下下码」，三个都能点击复制。
...
| 一键复制 | 点击验证码即复制到剪贴板 |
| 搜索 | 输入即按标题或网址过滤列表 |
| 当前码 + 下一码 + 下下码 | 鼠标悬停浮层同时展示三个码（当前/下一/下下），均可点击复制，避免码临过期来不及粘贴 |
...
| 自定义弹窗 | 用自定义对话框替代浏览器原生 `alert`/`confirm`，体验更一致 |
...
- **点击验证码** → 复制到剪贴板。
- **鼠标悬停在验证码上** → 弹出浮层，同时显示「当前码（蓝色）」「下一码（绿色）」「下下码（灰色）」，点击任意一个即可复制。
- **搜索** → 在搜索框输入，按标题或网址过滤列表。
...
- **未来码预览**：`getCodeAhead(secret, n)` 临时覆写 `Date.now()` 模拟 n 个周期后的时刻来生成对应码（用于浮层的下一码/下下码预览，不影响主流程）。
